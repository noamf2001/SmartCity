...
def get_points_between_two_points(point1, point2):
    ...
    for node in nodes: # 1184053312
        ...
        if name_tag:
            ...
            points.append(Point(lat,lon, node.get("uidZ")))
    return points
...
if __name__ == '__main__':
    i = 10
    for i in range(21):
        first = entrance_to_exact_science[i]
        second = entrance_to_exact_science[i+1]

        res = get_points_between_two_points(Point(first[1], first[0], -1),
                                        Point(second[1], second[0], -1))
        if(len(res) > 0):
            print(res)